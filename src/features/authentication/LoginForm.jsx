...
function LoginForm() {
  ...
  const scope =
    "user-follow-read user-read-email user-read-private user-library-read playlist-read-private user-read-recently-played user-read-playback-state user-read-currently-playing user-library-modify user-follow-modify playlist-modify-public playlist-modify-private ugc-image-upload user-top-read user-modify-playback-state user-read-playback-state streaming";
  ...
  function handleLoginClick() {
    ...
    if (refreshToken) navigate("/");
      ...
      window.location.href = `https://accounts.spotify.com/authorize?client_id=${import.meta.env.VITE_CLIENT_ID}&response_type=${RESPONSE_TYPE}&redirect_uri=${encodeURIComponent(import.meta.env.VITE_REDIRECT_URI)}&scope=${scope}&show_dialog=true`;
  }
