  ...
  return (
    <div className="flex items-center gap-2">
      {options.map((opt, index) => (
        <div
          key={index}
          className={`${checkFilter(opt.value) ? "group flex items-center gap-1 bg-blue-600 text-white" : "bg-white/50 text-black hover:scale-105 hover:bg-white/70 dark:bg-black/50 dark:text-white dark:hover:bg-black/70"} cursor-pointer rounded-full px-2.5 py-1 text-sm shadow`}
